// GENERAL workload reminder (Mon 17:00): emails only admins who haven't submitted this week.
...
function buildReminder(name:string,link:string,prettyDate:string){
  ...
  <div style="display:none;font-size:1px;line-height:1px;max-height:0;max-width:0;opacity:0;overflow:hidden;mso-hide:all">Reminder: your weekly workload check-in (${prettyDate}) is due today at 17:30.&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;</div>
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#F2F2F2"><tr><td align="center" style="padding:24px 12px">
    <table role="presentation" width="560" cellpadding="0" cellspacing="0" border="0" style="width:560px;max-width:100%">
      <tr><td bgcolor="${BLUE}" style="background-color:${BLUE};padding:24px 28px">
        ...
        <div style="font-family:Arial,Helvetica,sans-serif;color:#ffffff;font-size:20px;font-weight:bold;padding-top:6px">Reminder — Weekly Workload Check-in</div>
      </td></tr>
      <tr><td bgcolor="#ffffff" style="background-color:#ffffff;padding:26px 28px;border-left:1px solid #E2DDD9;border-right:1px solid #E2DDD9;border-bottom:1px solid #E2DDD9;font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#101418;line-height:1.5">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td bgcolor="#FFF7E0" style="background-color:#FFF7E0;border-left:4px solid ${AMBER};padding:12px 14px;font-size:13px;color:#7a5b00">This is a friendly reminder — your workload check-in for this week hasn't been submitted yet.</td></tr></table>
...
Deno.serve(async (req) => {
  try {
    ...
    const { data: formsLog } = await supabase.from("send_log").select("id").eq("kind","general_forms").eq("run_date",now.date).maybeSingle();
    if(!formsLog && !force) return new Response(JSON.stringify({skipped:true,reason:"no general forms sent today"}),{status:200});

    let reportDate = now.date;
    const { data: todays } = await supabase.from("general_submissions").select("id").eq("report_date",now.date).limit(1);
    if(!todays?.length){
    ...
    const { data: remLog } = await supabase.from("send_log").select("id").eq("kind","general_reminder").eq("run_date",reportDate).maybeSingle();
    if(remLog && !force) return new Response(JSON.stringify({skipped:true,reason:"reminder already sent"}),{status:200});

    const { data: pending } = await supabase.from("general_submissions").select("token, general_admins!inner(name,email,active)").eq("report_date",reportDate).eq("status","pending");
    const targets = (pending||[]).filter((r:any)=>r.general_admins?.active);
    if(!targets.length){
      await supabase.from("send_log").upsert({kind:"general_reminder",run_date:reportDate,detail:"no non-submitters"},{onConflict:"kind,run_date"});
    ...
    for(const t of targets){
      ...
      const r=await sendEmail(settings,t.general_admins.email,`Reminder — Weekly Workload Check-in due today 17:30 — ${prettyDate}`,buildReminder(t.general_admins.name,link,prettyDate));
      results.push({email:t.general_admins.email,...r});
    }
    await supabase.from("send_log").upsert({kind:"general_reminder",run_date:reportDate,detail:JSON.stringify(results)},{onConflict:"kind,run_date"});

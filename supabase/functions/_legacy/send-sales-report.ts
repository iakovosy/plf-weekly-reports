// SALES weekly summary email + branded PDF (Wed 08:00). Unicode-safe PDF (DejaVu Sans + fallback).
// Reliability (matches send-general-report v6):
//  - Fonts cached at module scope with an 8s timeout + one retry, falling back to Helvetica.
...
//  - Only a genuine success writes send_log kind "sales_report". Failures go to
//    "sales_report_error", which does NOT block a retry.
// Recipients come from summary_recipients (stream='sales', active) with fallback to the
// legacy sales_report_recipient setting if the table is empty.
...
async function logError(runDate:string,detail:string){
  try{ await supabase.from("send_log").upsert({kind:"sales_report_error",run_date:runDate,detail:String(detail).slice(0,500)},{onConflict:"kind,run_date"}); }catch(_e){}
}
...
async function buildPdf(prettyDate:string, submitted:any[], missingNames:string[]){
  ...
  const blue=rgb(0x4f/255,0x75/255,0xff/255),grey=rgb(.89,.87,.85),soft=rgb(.93,.95,1),black=rgb(.06,.08,.09);
  const W=842,H=595,M=36;
  const cols=[{t:"Admin",w:120},{t:"Workload",w:60},{t:"Conversions",w:90},{t:"Company reg. conv.",w:100},{t:"Assistance",w:60},{t:"Subject",w:130},{t:"Notes / feedback",w:150}];
  ...
  page.drawText("Sales Department — Weekly Report Summary",{x:M+70,y:H-46,size:15,font:bold,color:rgb(1,1,1)});
  page.drawText(`Week of ${prettyDate}`,{x:M+70,y:H-62,size:10,font,color:rgb(.92,.94,1)});
  let y=H-96;
  ...
  y-=18;
  const headerRow=()=>{let x=M;page.drawRectangle({x:M,y:y-4,width:cols.reduce((s,c)=>s+c.w,0),height:18,color:blue});for(const c of cols){page.drawText(c.t,{x:x+4,y,size:7.5,font:bold,color:rgb(1,1,1)});x+=c.w;}y-=20;};
  headerRow();
  let ri=0;
  for(const r of submitted){
    const a=r.answers||{};
    const vals=[raw(r.name),raw(a.workload_capacity),raw(a.conversions),raw(a.company_reg_conversions),ynRaw(a.needs_assistance==="true"||a.needs_assistance===true),raw(a.assistance_subject),raw(a.additional_notes)].map(clean);
    const cl=vals.map((v,i)=>wrap(v,cols[i].w,8,i===0?bold:font));
    const rowH=Math.max(...cl.map(l=>l.length))*10+6;
    if(y-rowH<50){page=doc.addPage([W,H]);y=H-50;headerRow();}
    if(ri%2===1)page.drawRectangle({x:M,y:y-rowH+12,width:cols.reduce((s,c)=>s+c.w,0),height:rowH,color:soft});
    let x=M;vals.forEach((_,i)=>{cl[i].forEach((line,li)=>{page.drawText(line,{x:x+4,y:y-li*10,size:8,font:i===0?bold:font,color:black});});x+=cols[i].w;});
    page.drawLine({start:{x:M,y:y-rowH+10},end:{x:M+cols.reduce((s,c)=>s+c.w,0),y:y-rowH+10},thickness:.5,color:grey});
    y-=rowH;ri++;
  }
  page.drawText("Generated automatically for the weekly Sales meeting — All Rights Reserved © Philippou Law Firm",{x:M,y:28,size:7.5,font,color:rgb(.6,.6,.6)});
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // At or past the scheduled hour on the scheduled day, so a failed run retries next tick.
    ...
    runDate = reportDate;
    const { data: logRow } = await supabase.from("send_log").select("id").eq("kind","sales_report").eq("run_date",reportDate).maybeSingle();
    if(logRow && !force) return new Response(JSON.stringify({skipped:true,reason:"already sent"}),{status:200});

    const { data: rows } = await supabase.from("sales_submissions").select("status, answers, sales_admins!inner(name,email)").eq("report_date",reportDate);
    const list = (rows||[]).map((r:any)=>({status:r.status,answers:r.answers,name:r.sales_admins.name})).sort((a:any,b:any)=>a.name.localeCompare(b.name));
    const [y,m,dd]=reportDate.split("-");const prettyDate=`${dd}/${m}/${y}`;
    const submitted=list.filter((r:any)=>r.status==="submitted");
    const missing=list.filter((r:any)=>r.status!=="submitted");
    const needHelp=submitted.filter((r:any)=>r.answers?.needs_assistance==="true"||r.answers?.needs_assistance===true).map((r:any)=>r.name);
    ...
    const td=(v:string,hl=false,b=false)=>`<td bgcolor="${hl?SOFT:"#ffffff"}" style="padding:8px 10px;border:1px solid ${GREY};font-size:12.5px;white-space:nowrap;font-family:Arial,Helvetica,sans-serif;background-color:${hl?SOFT:"#ffffff"};${b?"font-weight:bold;":""}">${v}</td>`;
    const tdW=(v:string)=>`<td bgcolor="#ffffff" style="padding:8px 10px;border:1px solid ${GREY};font-size:12.5px;min-width:140px;font-family:Arial,Helvetica,sans-serif;background-color:#ffffff">${v}</td>`;
    let table=`<table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;width:100%"><tr>`+th("Admin")+th("Workload")+th("Conversions")+th("Company reg. conv.")+th("Assistance")+th("Subject")+th("Notes / feedback")+`</tr>`;
    for(const r of submitted){const a=r.answers||{};const help=a.needs_assistance==="true"||a.needs_assistance===true;
      table+=`<tr>`+td(esc(r.name),false,true)+td(esc(a.workload_capacity),Number(a.workload_capacity)>=8)+td(esc(a.conversions))+td(esc(a.company_reg_conversions))+td(yn(help),help)+tdW(esc(a.assistance_subject))+tdW(esc(a.additional_notes))+`</tr>`;}
    table+="</table>";

    const html=`<!DOCTYPE html><html><body style="margin:0;padding:0;background:#F2F2F2">
  <div style="display:none;font-size:1px;line-height:1px;max-height:0;max-width:0;opacity:0;overflow:hidden;mso-hide:all">Sales weekly summary — ${submitted.length}/${list.length} submitted for ${prettyDate}.&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;</div>
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#F2F2F2"><tr><td align="center" style="padding:24px 12px">
    ...
    stage="build pdf";
    const pdfB64=await buildPdf(prettyDate,submitted,missing.map((r:any)=>r.name));
    const pdfName=`PLF-Sales-Weekly-Report-${reportDate}.pdf`;

    stage="resolve recipients";
    const { data: recs } = await supabase.from("summary_recipients").select("email").eq("stream","sales").eq("active",true);
    let to=(recs||[]).map((r:any)=>r.email).filter(Boolean);
    if(!to.length && (settings.sales_report_recipient||settings.report_recipient)) to=[settings.sales_report_recipient||settings.report_recipient];
    if(!to.length){ await logError(reportDate,"no active sales recipients"); return new Response(JSON.stringify({skipped:true,reason:"no active sales recipients"}),{status:200}); }

    stage="send";
    const r=await sendEmail(settings,to,`Sales Weekly Report Summary — ${prettyDate} (${submitted.length}/${list.length} submitted)`,html,pdfB64,pdfName);
    // Only a real success closes the week out; failures are recorded separately so the
    // next hourly tick retries.
    if(r.ok) await supabase.from("send_log").upsert({kind:"sales_report",run_date:reportDate,detail:r.detail},{onConflict:"kind,run_date"});
    else await logError(reportDate,"send failed: "+r.detail);
    return new Response(JSON.stringify({ok:r.ok,reportDate,to,detail:r.detail}),{headers:{"Content-Type":"application/json"}});
  } catch(e){

// GENERAL WEEKLY WORKLOAD REPORT — form sender (Mon 09:00 or forced). Individual via body.admin_id.
// Honours general_start_date. Outlook-safe email.
...
function buildEmail(name:string,link:string,prettyDate:string){
  ...
  <div style="display:none;font-size:1px;line-height:1px;max-height:0;max-width:0;opacity:0;overflow:hidden;mso-hide:all">Please complete this week's workload check-in (${prettyDate}) before Monday 17:30.&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;</div>
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#F2F2F2"><tr><td align="center" style="padding:24px 12px">
    <table role="presentation" width="560" cellpadding="0" cellspacing="0" border="0" style="width:560px;max-width:100%">
      <tr><td bgcolor="${BLUE}" style="background-color:${BLUE};padding:24px 28px">
        ...
        <div style="font-family:Arial,Helvetica,sans-serif;color:#ffffff;font-size:20px;font-weight:bold;padding-top:6px">Weekly Workload Check-in</div>
      </td></tr>
      <tr><td bgcolor="#ffffff" style="background-color:#ffffff;padding:26px 28px;border-left:1px solid #E2DDD9;border-right:1px solid #E2DDD9;border-bottom:1px solid #E2DDD9;font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#101418;line-height:1.5">
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td align="center" style="padding:24px 0 26px">
          <table role="presentation" cellpadding="0" cellspacing="0" border="0"><tr><td bgcolor="${BLUE}" style="background-color:${BLUE};border-radius:8px;padding:14px 38px" align="center">
            <a href="${link}" target="_blank" style="font-family:Arial,Helvetica,sans-serif;font-size:15px;font-weight:bold;color:#ffffff;text-decoration:none"><span style="color:#ffffff">Complete workload check-in</span></a>
          </td></tr></table>
        </td></tr></table>
...
Deno.serve(async (req) => {
  try {
    ...
    if (targetAdminId) {
      const { data: admin } = await supabase.from("general_admins").select("id,name,email").eq("id",targetAdminId).maybeSingle();
      ...
      await supabase.from("general_submissions").upsert([{admin_id:admin.id,report_date:reportDate}],{onConflict:"admin_id,report_date",ignoreDuplicates:true});
      const { data: sub } = await supabase.from("general_submissions").select("token").eq("admin_id",admin.id).eq("report_date",reportDate).single();
      const [y,m,d]=reportDate.split("-");const prettyDate=`${d}/${m}/${y}`;
      const link=`${formUrl}?token=${sub!.token}`;
      const r=await sendEmail(settings,admin.email,`Weekly Workload Check-in — ${prettyDate}`,buildEmail(admin.name,link,prettyDate));
    ...
    if(!force && startDate && now.date < startDate) return new Response(JSON.stringify({skipped:true,reason:"before general_start_date",now:now.date,startDate}),{status:200});

    const reportDate = now.date;
    const { data: logRow } = await supabase.from("send_log").select("id").eq("kind","general_forms").eq("run_date",reportDate).maybeSingle();
    if(logRow && !force) return new Response(JSON.stringify({skipped:true,reason:"already sent"}),{status:200});
    const { data: admins } = await supabase.from("general_admins").select("id,name,email").eq("active",true);
    if(!admins?.length) return new Response(JSON.stringify({skipped:true,reason:"no active general admins"}),{status:200});

    await supabase.from("general_submissions").upsert(admins.map((a:any)=>({admin_id:a.id,report_date:reportDate})),{onConflict:"admin_id,report_date",ignoreDuplicates:true});
    const { data: subs } = await supabase.from("general_submissions").select("admin_id,token").eq("report_date",reportDate);
    const tokenByAdmin = new Map(subs!.map((s:any)=>[s.admin_id,s.token]));
    const [y,m,d]=reportDate.split("-");const prettyDate=`${d}/${m}/${y}`;
    const results:any[]=[];
    for(const a of admins){
      const link=`${formUrl}?token=${tokenByAdmin.get(a.id)}`;
      const r=await sendEmail(settings,a.email,`Weekly Workload Check-in — ${prettyDate}`,buildEmail(a.name,link,prettyDate));
      results.push({email:a.email,...r});
    }
    await supabase.from("send_log").upsert({kind:"general_forms",run_date:reportDate,detail:JSON.stringify(results)},{onConflict:"kind,run_date"});
